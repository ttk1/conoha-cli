...
def auth_command(command):
    ...
    command.subcommand(name="auth").subcommand(
        name="login", description="API トークンを取得して token.json に保存する"
...
def image_command(command):
    ...
    command.subcommand(name="image").subcommand(
        name="list", description="イメージの一覧を表示する"
...
def server_command(command):
    ...
    command.subcommand(name="server").subcommand(
        ...
        "--admin-pass",
        help="VM の root パスワードを指定（使用可能な文字については要確認）",
    ).add_argument(
        # あらかじめ公開鍵を登録しておく必要あり！
        "--key-name",
        help="SSH キーの名前を指定",
        ...
        # ファイル指定でも良いかもしれない
        "--user-data",
        help="スタートアップスクリプトを指定（BASE64 エンコードはしなくてよい）",
    ).set_handler(
    ...
    command.subcommand(name="server").subcommand(
        name="start", description="VM を起動する"
    ).add_argument("--server-id", help="サーバーID", required=True).set_handler(
        server.server_start
    )

    # conoha server stop --server-id SERVER_ID
    command.subcommand(name="server").subcommand(
        name="stop", description="VM を停止する"
    ).add_argument("--server-id", help="サーバーID", required=True).add_argument(
    ...
    command.subcommand(name="server").subcommand(
        name="delete", description="VM を削除する"
    ).add_argument("--server-id", help="サーバーID", required=True).add_argument(
        "-f",
        "--force",
        action="store_true",
        help="削除ロックがかかっていても削除を強行する",
    ).set_handler(
    ...
    # conoha server list
    command.subcommand(name="server").subcommand(
        name="list", description="VM の一覧を表示する"
    ).add_argument(
    ...
    command.subcommand(name="server").subcommand(
        name="search", description="VM をネームタグ名の部分一致で検索する"
    ).add_argument(
    ...
    command.subcommand(name="server").subcommand(
        name="describe", description="VM の詳細を表示する"
    ).add_argument("--server-id", help="サーバーID", required=True).set_handler(
        server.server_describe
    )

    # conoha server attach-port --server-id SERVER_ID --port-id PORT_ID
    command.subcommand(name="server").subcommand(
        name="attach-port", description="VM にポートをアタッチする"
    ).add_argument("--server-id", help="サーバーID", required=True).add_argument(
        "--port-id", help="ポートID", required=True
    ).set_handler(
        server.server_attach_port
    )

    # conoha server detach-port --server-id SERVER_ID --port-id PORT_ID
    command.subcommand(name="server").subcommand(
        name="detach-port", description="VM からポートをでタッチする"
    ).add_argument("--server-id", help="サーバーID", required=True).add_argument(
        "--port-id", help="ポートID", required=True
    ).set_handler(
        server.server_detach_port
    )

    # conoha server list-ports --server-id SERVER_ID
    command.subcommand(name="server").subcommand(
        name="list-ports", description="VM にアタッチされているポートの一覧を表示する"
    ).add_argument("--server-id", help="サーバーID", required=True).set_handler(
        server.server_list_ports
    )
...
def subnet_command(command):
    ...
    command.subcommand(name="subnet").subcommand(
        name="create", description="サブネットを作成する"
    ).add_argument(
        # ローカルネットワークには高々１つのサブネットしか割り当てることが出来ない
        "--network-id",
        help="ローカルネットワークのnetwork_idを指定する",
        required=True,
    ).add_argument(
        "--cidr",
        help="プライベートアドレスのみ指定できます。bitmaskは21~27の間が指定できます。",
        required=True,
    ).set_handler(
    ...
    command.subcommand(name="subnet").subcommand(
        name="delete", description="サブネットを削除する"
    ).add_argument("--subnet-id", help="サブネットID", required=True).set_handler(
    ...
    command.subcommand(name="subnet").subcommand(
        name="describe", description="サブネットの詳細を表示する"
    ).add_argument("--subnet-id", help="サブネットID", required=True).set_handler(
        subnet.subnet_describe
    )
...
def security_group_command(command):
    ...
    command.subcommand(name="security-group").subcommand(
        name="create", description="セキュリティグループを作成する"
    ).add_argument(
        "--name",
        help="A symbolic name for the security group. 名前の重複はできません。",
        required=True,
    ).add_argument(
    ...
    command.subcommand(name="security-group").subcommand(
        ...
        security_group.security_group_delete
    )

    # conoha security-group list
    command.subcommand(name="security-group").subcommand(
        name="list", description="セキュリティグループの一覧を表示する"
    ).set_handler(security_group.security_group_list)

    # conoha security-group describe --group-id GROUP_ID
    command.subcommand(name="security-group").subcommand(
        name="describe", description="セキュリティグループの詳細を表示する"
    ).add_argument(
        "--security-group-id", help="セキュリティグループID", required=True
    ).set_handler(
        security_group.security_group_describe
    )
    ...
    command.subcommand(name="security-group").subcommand(
        ...
        "--port-range-min",
        help="セキュリティグループルールを範囲で設定する場合の最小ポート番号",
    ).add_argument(
        "--port-range-max",
        help="セキュリティグループルールを範囲で設定する場合の最大ポート番号",
    ).add_argument(
        "--protocol",
        choices=["tcp", "udp", "icmp", "null"],
        help="セキュリティグループルールが設定されるプロトコル。null を指定した場合すべての protocol を許可",
    ).add_argument(
        "--remote-group-id",
        help="指定したセキュリティグループIDに紐付いたポートからのトラフィックのみを許可",
    ).add_argument(
        "--remote-ip-prefix",
        help="指定した prefix を持つ IP からのトラフィックのみを許可",
    ).set_handler(
        security_group.secutiry_group_create_rule
    )

    # conoha security-group delete-rule --rule-id RULE_ID
    command.subcommand(name="security-group").subcommand(
        name="delete-rule", description="セキュリティグループルールを削除する"
    ).add_argument(
        "--rule-id", help="セキュリティグループルールID", required=True
    ).set_handler(
        security_group.security_group_delete_rule
    )

    # conoha security-group list-rules
    command.subcommand(name="security-group").subcommand(
        name="list-rules", description="セキュリティグループルールの一覧を表示する"
    ).set_handler(security_group.security_group_list_rules)

    # conoha security-group describe-rule --rule-id RULE_ID
    command.subcommand(name="security-group").subcommand(
        name="describe-rule", description="セキュリティグループの詳細を表示する"
    ).add_argument(
        "--rule-id", help="セキュリティグループルールID", required=True
    ).set_handler(
        security_group.security_group_describe_rule
    )
...
def network_command(command):
    ...
    command.subcommand(name="network").subcommand(
        name="create", description="ローカル通信用ネットワークを作成する"
    ).set_handler(network.network_create)

    # conoha network delete --network-id NETWORK_ID
    command.subcommand(name="network").subcommand(
        name="delete", description="ネットワークを削除する"
    ).add_argument("--network-id", help="ネットワークID", required=True).set_handler(
        network.network_delete
    )

    # conoha network list
    command.subcommand(name="network").subcommand(
        name="list", description="ネットワークの一覧を表示する"
    ).add_argument(
        "-l",
        "--local-only",
        action="store_true",
        help="ローカルネットワークのみを表示する",
    ).set_handler(
    ...
    command.subcommand(name="network").subcommand(
        name="describe", description="ネットワークの詳細を表示する"
    ).add_argument("--network-id", help="ネットワークID", required=True).set_handler(
        network.network_describe
    )
...
def port_command(command):
    ...
    command.subcommand(name="port").subcommand(
        name="create", description="ポートを作成する。固定 IP は必須。"
    ).add_argument("--network-id", help="ネットワークID", required=True).add_argument(
        ...
        "--security-group-ids",
        nargs="+",
        help="指定がない場合はDefaultのセキュリティグループが設定される",
    ).set_handler(
        port.port_create
    )
    ...
    ).add_argument("--port-id", help="ポートID", required=True).add_argument(
        "--security-group-ids",
        nargs="*",
        metavar="SECURITY_GROUP_ID",
        help="セキュリティグループID のリスト",
        required=True,
    ).set_handler(
        port.port_update
    )
    ...
    command.subcommand(name="port").subcommand(
        name="delete", description="ポートを削除する"
    ).add_argument("--port-id", help="ポートID", required=True).set_handler(
        port.port_delete
    )

    # conoha port list
    command.subcommand(name="port").subcommand(
        name="list", description="ポートの一覧を表示する"
    ).set_handler(port.port_list)

    # conoha port describe --port-id PORT_ID
    command.subcommand(name="port").subcommand(
        name="describe", description="ポートの詳細を表示する"
    ).add_argument("--port-id", help="ポートID", required=True).set_handler(
        port.port_describe
    )
